...
<Update label="Unreleased" description="Shipper, consignee, and other party roles accepted when creating a tracking request" rss="Unreleased: POST /v2/tracking_requests accepts shipper, consignee, notify_party, customs_broker, freight_forwarder, and pickup_dray_carrier relationships.">
  ### Party roles on tracking request creation

  `POST /v2/tracking_requests` accepts `shipper`, `consignee`, `notify_party`, `customs_broker`, `freight_forwarder`, and `pickup_dray_carrier` relationships next to `customer`. One party per role. The roles are copied to the shipment when it is created.
...
<Update label="September 2, 2026" description="Party role endpoints documented: assign shipper, consignee, freight forwarder, and other roles on shipments and containers through the API" rss="September 2, 2026: Documented the party role endpoints for shipments and containers, plus the flag[parties] query parameter on shipments.">
  ### Party role endpoints
...
<Update label="August 10, 2026" description="New Delivery Order workflow, OOCL tracker moved to the CargoSmart SCCT API, new Georgia Ports Authority Savannah API tracker, redesigned POD LFD cell with source chips and a detail panel, Complete Plan contract product in admin, container-level Dray Carrier in the expanded row, restored admin container delete from a B/L, wider carrier release-status coverage, and fixes across CMA CGM POL and barge events, YMLU last free days, Maersk missing containers, Full In / Empty Out stamping, locode resolution, TraPac LA future-dated updates, HMM container lookups, Goport batching, and OOLU CFFI resilience" rss="August 10, 2026: New Delivery Order workflow — draft, send, and track carrier acknowledgement through a reviewable link — plus DO view actions from the container modal and pickup triage; OOCL/OOLU tracker migrated to the CargoSmart SCCT JSON API; new Georgia Ports Authority Savannah (USSAV) API tracker; POD LFD cell redesigned as a date with per-source chips (Shipping line / Terminal, Est. / Manual), a coverage detail panel, and sortable Smart LFD; calculated LFDs now computed for every account (display still gated by opt-in, inland gated behind smart_lfd_ind); user-entered LFDs reported with a manual selection so cells label them accordingly; Complete Plan added to the admin contract product catalog; container-level Dray Carrier now assignable from the expanded container row; admin container delete from a B/L restored via V2 destroy; Terminal49 support accounts can now read container terminal history; hold detail now carries each source's own wording; terminal line holds routed into holds v2 with an admin toggle and active-holds backfill; expanded carrier release-status parsing across COSU, HDMU, MATS, ONEY, and YMLU trackers; Pickup Dray Carrier renamed to Dray Carrier across dashboard columns, filters, bulk upload, and CSV export; PartyRole is now the source of truth for customer, with new customers created as parties without a linked account and customer id inputs accepting both party and account ids; party contact and location editing from the new side panel; and fixes for CMA CGM barge events (ABA/ABD) and POL resolution from the transport call, YMLU last free days placed by carrier label, Maersk payloads with no containers key, Maersk event location state, Full In / Empty Out timestamps stamped away from the POL, exact locode beating a fuzzy city guess, locode aliases applied to inland locations, Cplus CSX code mapped to DP World Hong Kong CT3, future-dated TraPac LA terminal updates, HMM container lookup crash on invalid numbers, GCT Canada !NOBLIMPORT hold description, Goport partial-batch failures and non-JSON responses, HTTPI SSL/timeout classification, GO! Port password-expiry detection, and OOLU CFFI failure classification and Cloudflare ticket retries.">
  ...
  - **New customers can be parties without a linked account** — creating a customer no longer implicitly creates a shipper Account; customers without a linked account are exposed under their party id (customers with a linked account keep exposing their account id)
  ...
  - **Customer dropdown is stable across reloads** — dashboards read customers through the parties endpoint so customers without a linked account stay visible after refresh
