...
<Steps>
  <Step title="Pick your MCP client">
    ...
    - [Claude](/mcp/setup/claude) (claude.ai, Claude Desktop — [available in the Claude Directory](https://claude.ai/directory/connectors/terminal49))
  ...
  <Step title="Add the connector URL">
    For Claude, add Terminal49 from the [Claude Directory](https://claude.ai/directory/connectors/terminal49). For other clients, point the client at `https://mcp.terminal49.com`. No API key needed.
  </Step>
...
## Query other Terminal49 data

The `query` tool reads Rails v2 API endpoints with the same account and user credentials used by the other MCP tools. For example, use `path: "/documents"` with `params: {"page[size]": "10"}` to request a page of documents, or `path: "/containers/CONTAINER_ID/transport_events"` for a container timeline. The path begins after `/v2`; pass query parameters separately.

`query` only sends GET requests. Rails decides which endpoints and records the signed-in caller can access. Results include the Rails response and pagination links. Follow those links when a question needs more than one page, and request smaller pages when a response exceeds the tool's size limit.

For containers arriving in Los Angeles during a chosen week, request `/containers` with `filter[pod_code]=USLAX`, two `filter[arrival][]` bounds (`>=START_DATE` and `<=END_DATE`), `sort=arrival`, and `page[size]=25`. Include `USLGB` only when you mean the wider Los Angeles/Long Beach port complex. State the dates used, separate upcoming ETAs from recorded arrivals, and follow additional pages before describing the result as complete.
...
See [Entitlements and Paid Features](/api-docs/useful-info/entitlements) for the related Routing Data entitlement.
</Warning>
...
| Webhooks | Real-time event notifications | Configure via the [webhooks guide](/api-docs/in-depth-guides/webhooks) |
