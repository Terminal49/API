...
Instead of adding the MCP server directly, you can install the **Terminal49** plugin from the [Terminal49/agent-plugins](https://github.com/Terminal49/agent-plugins) marketplace. The plugin configures the same MCP connection (`https://mcp.terminal49.com`) and adds a `container-tracking` skill that teaches Claude Code how to choose and sequence the Terminal49 tools — covering status, pickup readiness, holds, ETAs, routes, delays, and demurrage-risk questions.
...
Authentication works the same way: the first time the Terminal49 tools are used, sign in through the browser OAuth flow (run `/mcp` and choose **Authenticate** if you aren't prompted). The plugin contains no API keys or customer data. See the [repository README](https://github.com/Terminal49/agent-plugins#readme) for details.
