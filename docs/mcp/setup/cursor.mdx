...
<Note>
**No API key required.** The Terminal49 MCP server supports OAuth 2.1. When you connect, Cursor opens a browser sign-in page. Log in with your Terminal49 credentials to finish.
</Note>
...
<Steps>
  <Step title="Add the Terminal49 server">
    The fastest way is the one-click install button. It opens Cursor with the Terminal49 server configuration pre-filled; confirm to install:
