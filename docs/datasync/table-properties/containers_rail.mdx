...
- a `timestamp` type column in the UTC time zone (Universal Time Coordinated).
- a `text` type column in the local time zone of where the event happened. The format is : YYYY-MM-DD HH:MI:SS. For example "2024-09-24 17:25:00" for 5:25 PM on September 24, 2024. Depending on the event, the time zone can be the one from the Port of Lading (`pol_timezone`), the Port of Discharge (`pod_timezone`), or the Inland Destination (`ind_timezone`).
...
| pod_last_free_day_on            | "Current Last Free Day at the POD terminal, as a UTC timestamp\n\nNamed `pickup_lfd` in the API."                      | timestamp |
| pod_last_free_day_on_local      | Current Last Free Day at the POD terminal, as a string in the POD local time zone                                      | text      |
| pod_pickup_appointment_at       | Port of Discharge Pickup Appointment, as a UTC timestamp (named pickup_appointment_at in the API)                      | timestamp |
...
| ind_arrived_at*                 | Inland Destination Actual Time of Arrival, as a UTC timestamp (named ind_ata_at in the API)                            | timestamp |
...
| ind_last_free_day_on*           | Last Free Day at the inland destination facility, as a UTC timestamp (named ind_facility_lfd_on in the API)            | timestamp |
...
| ind_full_out_at                 | Inland Destination Full Out event, as a UTC timestamp (named final_destination_full_out_at in the API)                 | timestamp |
