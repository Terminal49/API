...
- **Container terminal operators:** container availability, last free day, holds, fees, and related terminal data
- **Container rail carriers:** container milestones via rail
- **Vessel position data:** vessel details and location tracking for accounts entitled to the vessel, routing, and map features (see [Entitlements and Paid Features](/api-docs/useful-info/entitlements)). Coverage and freshness vary by vessel and data source.

## Supported ocean carriers

Coverage can vary by tracking-number type and source availability.
...
| Seaboard Marine | Seaboard | SMLU | - | On roadmap | - |
| Crowley | Crowley | CLAM | - | On roadmap | - |

<Tip>
  You can also use the [Infer Tracking
  Number](/api-docs/in-depth-guides/auto-detect-carrier) endpoint (Auto-Detect
  Carrier) to automatically identify the SCAC from a tracking number.
</Tip>

## Ports and terminals

The Terminal49 API integrates with terminals at the following ports:
...
The table below lists US terminals with their FIRMS codes. Terminal data availability can vary by field, terminal source, and shipment state.
...
| Everport Terminal Services | ETS | Y124 | Los Angeles | USLAX | Yes |
