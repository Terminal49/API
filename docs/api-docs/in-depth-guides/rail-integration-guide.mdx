...
For inland rail demurrage workflows, subscribe to `container.pickup_lfd_rail.changed` to detect changes to the rail terminal Last Free Day.
...
These attributes can be found on [container objects](/docs/api-docs/api-reference/containers/get-a-container).
