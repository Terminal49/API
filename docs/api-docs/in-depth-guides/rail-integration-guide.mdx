...
There are several core Transport Events that occur on most rail journeys.  Some rail carriers do not share all events, (see [Rail Carriers](/api-docs/useful-info/api-data-sources-availability#known-issues-rail) for more details), but in general these are the key events for a container.
