...
2. Replace `request_number` and `scac` with your shipment details. The request number must be a shipping line booking number, master bill of lading number, or container number. The SCAC must be a shipping line SCAC. See [Data Coverage](/coverage/home) for coverage details.

```json POST /tracking_requests
{
  "method": "post",
