...
To add a customer to a tracking request, include the party as a customer relationship when creating the request. This endpoint accepts the `customer` relationship only at creation. To add or change roles on an existing tracking request or shipment, including shipper and consignee, use [party roles](/api-docs/in-depth-guides/assigning-parties).
...
The response carries the tracking request ID. Its `customer` relationship references the party's linked account and is `null` when the party has none, which is the case for parties created through this API. To read the assigned party, use [`GET /v2/tracking_requests/TRACKING_REQUEST_ID/party_roles`](/api-docs/api-reference/party-roles/list-tracking-request-party-roles).
