...
Every tracking request requires two things: **your tracking number** and **the carrier's SCAC code**.
...
To track a shipment or container, Terminal49 needs to know **which vessel carrier (VOCC) to ask**.
The SCAC (Standard Carrier Alpha Code) we use here is the **VOCC SCAC for tracking** — i.e., the carrier operating the move we’re querying for events and shipment data.

| You Have                        | You Need                           | The Challenge                                                                                                               |
| ------------------------------- | ---------------------------------- | --------------------------------------------------------------------------------------------------------------------------- |
| Bill of Lading: `MAEU123456789` | VOCC SCAC (carrier operating move) | Many MBOLs **do not include a prefix**, and even when they do, it may not reliably identify the VOCC you need for tracking. |
| Container: `WHLU1234560`        | VOCC SCAC (carrier operating move) | The container owner code / leasing company is not always the carrier moving it, so the prefix alone is not enough.          |
| Booking: `987654321`            | VOCC SCAC (carrier operating move) | Booking formats vary widely and often contain no carrier identifier.                                                        |

<Info>
  Without the correct **VOCC SCAC**, your tracking request can fail even if the
  number is valid. The Infer API predicts the VOCC SCAC + number type to
...
- **The predicted carrier (SCAC)** — so you don't have to guess
...
<Tabs>
  <Tab title="Container Number" icon="box">
    Container numbers follow the ISO 6346 format. While the first three letters (owner code) often indicate the owner, the container might be moved by a different carrier (VOCC).

    Our system analyzes the number against tens of millions of historical records to predict which carrier is moving the container.
    ...
    **Example Input:** `MSCU1234567`
    ...
    <Tip>
      For container numbers, we use historical data to identify the carrier with high accuracy (9/10 times).
