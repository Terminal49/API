...
If your deploy exposes the rewrite, you can also call `https://mcp.terminal49.com/mcp`.

## Transports
- HTTP (streamable): `POST /api/mcp`
- SSE (stateful): `GET /sse?sessionId=<id>` to open the stream, `POST /sse?sessionId=<id>` to send messages on the same session.
