...
<Note>
Just want to get started fast? See [MCP Overview](/docs/mcp/home) for a 5-minute setup.
</Note>
...
Pass `Authorization: Token <T49_API_TOKEN>`. `Bearer <T49_API_TOKEN>` is accepted for backward compatibility, but `Token` is the canonical scheme.
...
| `401 Unauthorized` | Missing or invalid token | Create a new API token in [dashboard](https://app.terminal49.com/developers/api-keys); ensure `Authorization: Token <token>` header is set |
...
- [API Data Sources](/docs/api-docs/useful-info/api-data-sources-availability) – Data freshness and coverage
