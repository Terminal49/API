...
See [Container Holds, Fees, and Release Readiness](/docs/api-docs/in-depth-guides/holds-and-fees) for details on specific hold types and how to determine when the container is released.
...
### dropped
**Reserved** - Terminal49 does not currently emit this status in public API responses. Keep a fallback branch for unknown or newly introduced statuses rather than building workflow logic around `dropped`.

### loaded
**Reserved** - Terminal49 does not currently emit this status in public API responses. Keep a fallback branch for unknown or newly introduced statuses rather than building workflow logic around `loaded`.
