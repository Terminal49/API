...
<Note>
    Routing Data (Container Map GeoJSON API) is a paid feature. These APIs are subject to additional terms of usage and pricing. See [Entitlements and Paid Features](/api-docs/useful-info/entitlements) for the required entitlement and non-entitled error response.
</Note>
...
<AccordionGroup>
  <Accordion title="How up-to-date is the vessel position data?">
  ...
  <Accordion title="How accurate are the future predictions?">
  ...
  <Accordion title="What if a vessel deviates from the predicted path?">
  ...
  <Accordion title="Why don't I see a current_vessel feature for my container?">
  ...
  <Accordion title="Can I get map data for multiple containers at once?">
  ...
  <Accordion title="What coordinate system is used?">
    All coordinates follow the GeoJSON standard: `[longitude, latitude]` in WGS84 (EPSG:4326) format.
  </Accordion>
  <Accordion title="Are the LineString coordinates simplified?">
