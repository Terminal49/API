...
description: Submit documents by email or dashboard upload, then consume extraction results via webhooks.
...
- Email attachments to your docs alias
- Upload files in the Documents dashboard
...
```mermaid
flowchart LR
    A[User uploads document in dashboard] --> B[Terminal49 receives and parses document]
    B --> C[Terminal49 classifies document]
    C --> D[Terminal49 extracts structured data]
    D --> E[Terminal49 sends webhook result]
```
...
<Steps>
  <Step title="Submit document">
    Use email (attachments to your docs alias) or dashboard upload.
  </Step>
