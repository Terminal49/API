---
title: Assign a container party role
description: "Assign a party to a container in the Terminal49 API as pickup dray carrier. Roles are additive, and duplicate party-role pairs return a 422 error."
og:title: Assign a Container Party Role | Terminal49 API Documentation
og:description: Assign a party to a container in the Terminal49 API as dray carrier.
openapi: post /containers/{container_id}/party_roles
---

Attaches a party from your account to the container in one role.

## Path parameters

| Parameter | Required | Description |
|-----------|----------|-------------|
| `container_id` | Yes | The ID of the container |

## Roles

`pickup_dray_carrier` only

## Behavior

- Roles are additive. Posting a second party with the same role keeps both
- To replace a party, delete its role and create a new one
- Posting the same party and role twice returns `422`
- The party must belong to your account, otherwise the request returns `401`
- Any other role returns `422`
