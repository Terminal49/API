...
async function main() {
  ...
  if (process.env.T49_CONTAINER_STATUS) containerFilters['filter[status]'] = process.env.T49_CONTAINER_STATUS;
  if (process.env.T49_CONTAINER_PORT) containerFilters['filter[pod_locode]'] = process.env.T49_CONTAINER_PORT;
  const containers = await client.listContainers(containerFilters, { format: 'mapped' });
  logSection('containers', containers, 3);

  // Shipments (optionally filtered)
  const shipmentFilters: Record<string, string> = {};
  if (process.env.T49_SHIPMENT_STATUS) shipmentFilters['filter[status]'] = process.env.T49_SHIPMENT_STATUS;
  if (process.env.T49_SHIPMENT_PORT) shipmentFilters['filter[pod_locode]'] = process.env.T49_SHIPMENT_PORT;
  const shipments = await client.listShipments(shipmentFilters, { format: 'mapped' });
  logSection('shipments', shipments, 3);

  // Tracking requests
  const trackingFilters: Record<string, string> = {};
  const trackingRequests = await client.listTrackingRequests(trackingFilters, { format: 'mapped' });
