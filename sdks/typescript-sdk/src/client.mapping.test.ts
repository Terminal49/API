...
function findIncluded(doc: any, type: string, id: string | undefined): any | null {
  if (!id) return null;
  const included = Array.isArray(doc?.included) ? doc.included : [];
  return included.find((item: any) => item.type === type && item.id === id) || null;
}
...
describe('Terminal49Client mapping helpers', () => {
  it('maps shipping lines with optional fields', async () => {
    ...
    const expectedScacs = (fixture?.data || []).map(getShippingLineScac).filter(Boolean);
    ...
    const firstName = first?.attributes?.name || first?.attributes?.full_name || firstScac;
    ...
    if (withShort) {
      ...
      const expectedShort = withShort.attributes?.short_name || withShort.attributes?.nickname;
      expect(mapped?.shortName).toBe(expectedShort);
    }

    const withBolPrefix = fixture?.data?.find((item: any) => item?.attributes?.bol_prefix);
    if (withBolPrefix) {
    ...
    const withNotes = fixture?.data?.find((item: any) => item?.attributes?.notes);
    if (withNotes) {
  ...
  itIf(hasFixture('containers.route'))(
    ...
    async () => {
      ...
      const { fetchImpl } = createMockFetch({
        '/containers/cont-1/route?include=port,vessel,route_location': () => jsonResponse(fixture),
      });
      ...
      const routeLocationRef = fixture?.data?.relationships?.route_locations?.data?.[0];
      ...
      const port = locationId ? findIncluded(fixture, 'port', locationId) : null;
      if (port) {
        ...
        expectIfDefined(result.locations[0]?.port?.countryCode, port.attributes?.country_code);
      }

      expectIfDefined(result.locations[0]?.inbound?.mode, routeLocation.attributes?.inbound_mode);
      expectIfDefined(
        result.locations[0]?.inbound?.carrierScac,
        routeLocation.attributes?.inbound_scac,
      );
      expectIfDefined(result.locations[0]?.inbound?.eta, routeLocation.attributes?.inbound_eta_at);
      expectIfDefined(result.locations[0]?.inbound?.ata, routeLocation.attributes?.inbound_ata_at);

      expectIfDefined(result.locations[0]?.outbound?.mode, routeLocation.attributes?.outbound_mode);
      expectIfDefined(
      ...
      const inboundVesselId = routeLocation?.relationships?.inbound_vessel?.data?.id;
      const inboundVessel = inboundVesselId
        ? findIncluded(fixture, 'vessel', inboundVesselId)
        : null;
      if (inboundVessel) {
        expectIfDefined(result.locations[0]?.inbound?.vessel?.name, inboundVessel.attributes?.name);
        expectIfDefined(result.locations[0]?.inbound?.vessel?.imo, inboundVessel.attributes?.imo);
      }

      const outboundVesselId = routeLocation?.relationships?.outbound_vessel?.data?.id;
      ...
      if (outboundVessel) {
        ...
        expectIfDefined(result.locations[0]?.outbound?.vessel?.imo, outboundVessel.attributes?.imo);
      }
    },
  );
  ...
  itIf(hasFixture('tracking-requests.get.include'))(
    ...
    async () => {
      ...
      const shipment = shipmentRef ? findIncluded(fixture, 'shipment', shipmentRef.id) : null;
      const container = containerRef ? findIncluded(fixture, 'container', containerRef.id) : null;
      ...
      if (shipment) {
        ...
        expectIfDefined(result.shipment?.shippingLineScac, shipment.attributes?.shipping_line_scac);
      }
  ...
  it('maps shipment ports, terminals, tracking, and container references', async () => {
    ...
    const expectedBill = attrs.bill_of_lading_number || attrs.bill_of_lading || attrs.bl_number;
    ...
    expectIfDefined(result.tracking?.lineTrackingStoppedAt, attrs.line_tracking_stopped_at);
    expectIfDefined(result.tracking?.lineTrackingStoppedReason, attrs.line_tracking_stopped_reason);
    ...
    if (pol) {
      expectIfDefined(result.ports?.portOfLading?.locode, pol.attributes?.locode);
      ...
      expectIfDefined(result.ports?.portOfLading?.countryCode, pol.attributes?.country_code);
    }
    ...
    const podTerminal = podTerminalRef
      ...
      : null;
    if (podTerminal) {
      expectIfDefined(result.ports?.portOfDischarge?.terminal?.id, podTerminal.id);
      expectIfDefined(result.ports?.portOfDischarge?.terminal?.name, podTerminal.attributes?.name);
      expectIfDefined(
    ...
    if (podRef) {
      ...
      if (portOfDischarge) {
        expectIfDefined(result.ports?.portOfDischarge?.locode, portOfDischarge.attributes?.locode);
        expectIfDefined(result.ports?.portOfDischarge?.name, portOfDischarge.attributes?.name);
        expectIfDefined(result.ports?.portOfDischarge?.code, portOfDischarge.attributes?.code);
        expectIfDefined(
    ...
    expectIfDefined(result.ports?.portOfDischarge?.originalEta, attrs.pod_original_eta_at);
    expectIfDefined(result.ports?.portOfDischarge?.timezone, attrs.pod_timezone);

    if (attrs.destination_locode) {
      expectIfDefined(result.ports?.destination?.locode, attrs.destination_locode);
      ...
      expectIfDefined(result.ports?.destination?.timezone, attrs.destination_timezone);
    }

    const destinationTerminal = destinationTerminalRef
      ? findIncluded(fixture, destinationTerminalRef.type, destinationTerminalRef.id)
      : null;
    if (destinationTerminal) {
      expectIfDefined(result.ports?.destination?.terminal?.id, destinationTerminal.id);
      expectIfDefined(
  ...
  it('maps container list items from base fixture', async () => {
    ...
    const { fetchImpl } = createMockFetch({
      '/containers?include=shipment,pod_terminal,pickup_facility': () => jsonResponse(fixture),
    });
    ...
    expectIfDefined(result.items[0]?.number, attrs.number || attrs.container_number);
    ...
    expectIfDefined(result.items[0]?.location?.currentLocation, attrs.location_at_pod_terminal);
    expectIfDefined(result.items[0]?.location?.availableForPickup, attrs.available_for_pickup);
    expectIfDefined(result.items[0]?.location?.podArrivedAt, attrs.pod_arrived_at);
    expectIfDefined(result.items[0]?.location?.podDischargedAt, attrs.pod_discharged_at);

    expectIfDefined(result.items[0]?.demurrage?.pickupLfd, attrs.pickup_lfd);
    expectIfDefined(result.items[0]?.demurrage?.pickupAppointmentAt, attrs.pickup_appointment_at);
    if (Array.isArray(attrs.fees_at_pod_terminal)) {
      expect(result.items[0]?.demurrage?.fees).toEqual(attrs.fees_at_pod_terminal);
    }
    if (Array.isArray(attrs.holds_at_pod_terminal)) {
      expect(result.items[0]?.demurrage?.holds).toEqual(attrs.holds_at_pod_terminal);
    }

    expectIfDefined(result.items[0]?.rail?.podRailCarrierScac, attrs.pod_rail_carrier_scac);
    expectIfDefined(result.items[0]?.rail?.indRailCarrierScac, attrs.ind_rail_carrier_scac);
    expectIfDefined(result.items[0]?.rail?.podRailLoadedAt, attrs.pod_rail_loaded_at);
    expectIfDefined(result.items[0]?.rail?.podRailDepartedAt, attrs.pod_rail_departed_at);
    expectIfDefined(result.items[0]?.rail?.indRailArrivedAt, attrs.ind_rail_arrived_at);
    expectIfDefined(result.items[0]?.rail?.indRailUnloadedAt, attrs.ind_rail_unloaded_at);
  ...
  it('returns empty lists when container or shipment list data is not an array', async () => {
    const { fetchImpl } = createMockFetch({
      '/containers?include=shipment,pod_terminal,pickup_facility': () => jsonResponse({ data: {} }),
    ...
    const containers = (await client.listContainers({}, { format: 'mapped' })) as any;
    const shipments = (await client.listShipments({}, { format: 'mapped' })) as any;
  ...
  it('resolves every route leg port from the `location` relationship', async () => {
    ...
    expect(result.locations[0]?.outbound?.vessel?.name).toBe('MAERSK EDINBURGH');
  ...
  it('maps container current_status and pickup_facility from real paths', async () => {
    ...
    expect(result.terminals?.destinationTerminal?.name).toBe(pickup.attributes.name);
    expect(result.terminals?.destinationTerminal?.firmsCode).toBe(pickup.attributes.firms_code);

    expect(result.equipment?.type).toBe(attrs.equipment_type);
    expect(result.location?.availableForPickup).toBe(attrs.available_for_pickup);
  ...
  it('restores shipping-line alternative_scacs and tracking-support flags', async () => {
    ...
    const mappedAlt = result.find((line) => line.scac === sourceWithAlt.attributes.scac);
    expect(mappedAlt?.alternativeScacs).toEqual(sourceWithAlt.attributes.alternative_scacs);

    const sourceWithFlag = (fixture?.data || []).find(
      (item: any) => item?.attributes?.container_number_tracking_support === false,
    );
    expect(sourceWithFlag).toBeTruthy();
    const mappedFlag = result.find((line) => line.scac === sourceWithFlag.attributes.scac);
