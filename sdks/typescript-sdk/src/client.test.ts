...
function createMockFetch(handlers: Record<string, (init?: RequestInit, url?: URL) => Response>) {
  const calls: Array<{ init?: RequestInit; url: URL }> = [];

  const fetchImpl = async (input: Request | URL | string, init?: RequestInit) => {
    const request = input instanceof Request ? input : undefined;
    const urlString =
      typeof input === 'string' ? input : input instanceof URL ? input.toString() : request?.url ?? '';
...
describe('Terminal49Client', () => {
  ...
  it('sets include params on shipment and lists shipping lines with search', async () => {
    const { fetchImpl, calls } = createMockFetch({
      ...
      '/shipping_lines?search=MAEU': () =>
        jsonResponse({ data: [{ attributes: { scac: 'MAEU', name: 'Maersk' } }] }),
    });
  ...
  it('sends JSON:API payload when tracking container', async () => {
    ...
    await client.trackContainer({ containerNumber: 'MSCU1234567', scac: 'MSCU' });
  ...
  it('maps tracking request with linked shipment/container', async () => {
    const doc = {
      data: {
        id: 'tr-1',
        type: 'tracking_request',
        attributes: { request_type: 'container', request_number: 'MSCU1234567', status: 'created', scac: 'MSCU' },
        relationships: {
          shipment: { data: { type: 'shipment', id: 'ship-1' } },
      ...
      included: [
        {
          id: 'ship-1',
          type: 'shipment',
          attributes: { bill_of_lading_number: 'BL123', shipping_line_scac: 'MSCU' },
        },
        {
          id: 'cont-1',
          type: 'container',
    ...
    const result = await client.getTrackingRequest('tr-1', { format: 'mapped' });
      ...
      included: [
        {
          id: 'term-1',
          type: 'terminal',
          attributes: { name: 'Terminal 1', nickname: 'T1', firms_code: 'F123' },
        },
      ],
    };
  ...
  it('maps transport events with location/terminal', async () => {
    const doc = {
      data: [
        {
          ...
          attributes: { event: 'container.transport.vessel_loaded', event_time: '2024-01-01T00:00:00Z' },
          relationships: {
      ...
      included: [
        { id: 'loc-1', type: 'location', attributes: { name: 'Los Angeles', locode: 'USLAX' } },
        { id: 'term-1', type: 'terminal', attributes: { name: 'Yusen', nickname: 'YUS', firms_code: 'Y790' } },
      ],
    };
    ...
    const events = await client.getContainerTransportEvents('abc', { format: 'mapped' }) as any[];
  ...
  it('handles validation errors with proper message extraction', async () => {
    const { fetchImpl } = createMockFetch({
      '/tracking_requests': () =>
        jsonResponse(
          { errors: [{ detail: 'request_number is required', source: { pointer: '/data/attributes/request_number' } }] },
          400
        ),
    });
