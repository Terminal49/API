...
The REST API documentation can be found on [www.terminal49.com](https://www.terminal49.com). The full API of this library can be found in [api.md](api.md).
...
```sh
npm install git+ssh://git@github.com:stainless-sdks/terminal49-typescript.git
```
...
const response = await client.shipments.list().asResponse();
...
1. Via the `TERMINAL49_LOG` environment variable
...
const client = new Terminal49({
  logger: logger.child({ name: 'Terminal49' }),
...
const client = new Terminal49({ fetch });
```
...
```ts
import Terminal49 from 'npm:terminal49';
...
We are keen for your feedback; please open an [issue](https://www.github.com/stainless-sdks/terminal49-typescript/issues) with questions, bugs, or suggestions.
