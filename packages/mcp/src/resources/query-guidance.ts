...
export function readQueryGuidanceResource(): string {
  return [
    ...
    '- For an identified container or vessel, use search_container then get_container; use get_container_transport_events for delay context.',
    '- For "containers arriving in LA this week", use query path "/containers" with params {"filter[pod_code]":"USLAX","filter[arrival][]":[">=START_DATE","<=END_DATE"],"sort":"arrival","page[size]":"25"}. Replace START_DATE and END_DATE with the chosen calendar window.',
    '- Use USLGB as well only when the user means the wider LA/Long Beach port complex; do not silently expand LA to Long Beach.',
    '- State the date window and timezone, distinguish upcoming ETA from actual arrival, follow pagination links, and never claim completeness from one page.',
    '- The specialized list_containers tool does not forward port or arrival filters; use query for those Rails-supported filters.',
    '',
    ...
    '- The specialized list_containers tool has no server-side filter or sort. Order each returned page client-side by pickupLfd and surface holdsAtPodTerminal, but do not claim the page represents every at-risk container in the account. The query tool can forward supported Rails container filters and sorts.',
    '',
    '### 7) Shipment list filtering',
    '- list_shipments supports shipment number and tracking_stopped filters.',
    '- Those specialized shipment-list inputs do not forward status, port, carrier, or updated-time filters. The query tool can use Rails-supported filters on the relevant endpoint; verify the route and actual result before claiming a filter applied.',
    '',
